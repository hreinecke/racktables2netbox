...
class REST(object):
    ...
    def check_device(self, dev):
        ...
        print('Checking device from {}'.format(url))
        data = self.fetcher(url)
        return data
    ...
    def validate_qualys(self, filename):
        with open(filename) as file:
            ...
            for line in lines:
                ...
                host_id = x[1]
                dns_name = x[2]
                host_name = dns_name.split('.')[0]
                if '-sp' in host_name:
                    host_name = host_name[:len(host_name) - 3]
                print(f'Checking {asset_id} {host_id} {host_name} {dns_name}')
                hostdata = self.check_device(host_name)
                print(f'{hostdata}')
                if not hostdata['results']:
                    continue
                device = hostdata['results'][0]
...
if __name__ == '__main__':
    ...
    rest = REST()
    rest.validate_qualys('Nuremberg.csv')
